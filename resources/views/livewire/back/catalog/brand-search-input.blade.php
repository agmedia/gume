        ...
        <div class="autocomplete p-3" @if( ! $show_add_window) hidden @endif style="position:absolute; z-index:10; top:38px; background-color: #f6f6f6; border: 1px solid #d7d7d7;">
            <div class="row">
                ...
                <div class="mb-0 mt-1 col-md-12 text-right">
                    <a href="javascript:void(0)" wire:click="makeNewBrand" class="btn btn-primary btn-save shadow-sm">
        ...
        <div class="autocomplete pt-1" style="position:absolute; z-index:10; top:38px; background-color: #f6f6f6; border: 1px solid #d7d7d7;width:100%">
            <div id="myInputautocomplete-list" class="autocomplete-items">
                @foreach($search_results as $brand)
                    <div style="cursor: pointer;border-bottom: 1px solid #d7d7d7;padding-bottom: 10px;padding-left: 10px;font-size: 16px" wire:click="addBrand('{{ $brand->id }}')">
                        <small class="font-weight-lighter">Ime: <strong>{{ $brand->title }}</strong></small>
                    </div>
                @endforeach
            </div>
        </div>
